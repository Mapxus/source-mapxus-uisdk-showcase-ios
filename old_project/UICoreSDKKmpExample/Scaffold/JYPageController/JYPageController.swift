...
open class JYPageController: UIViewController {
    /// config
    public var config = JYPageConfig.init()
    ...
    private var childControllerCache = NSCache<NSString, UIViewController>()

    /// 缓存当前scrollView上展示的vc，用于处理子vc的生命周期逻辑
    var displayControllerCache = [NSString: UIViewController]()

    /// childController scrollView cache
    var childScrollViewCache = [NSString: UIScrollView]()
    ...
    deinit {
        childScrollViewCache.forEach { (_: NSString, value: UIScrollView) in
    ...
    lazy var menuView: JYPageMenuView = {
