...
: BaseRowFormer<T>, Formable, UpdatableSelectorForm where T: SelectorPickerFormableRow {
    ...
    open override func cellSelected(indexPath: IndexPath) {
        ...
        cell.selectorPickerView?.selectRow(indexPath.row, inComponent: 0, animated: false)
    }
    ...
    public func editingDidBegin() {
        if enabled {
            ...
            if let selectedRow = selectedRow, pickerItems[selectedRow].displayTitle == nil {
                ...
                selectorView.selectRow(selectedRow, inComponent: 0, animated: true)
            }
            isEditing = true
        }
    }
...
//            titleColor = nil
    ...
    fileprivate final lazy var observer = Observer<T, S>(selectorPickerRowFormer: self)
}
