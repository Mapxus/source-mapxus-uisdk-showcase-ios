...
//  Type: Double?
//  Default: nil
//  Description: Controls how strongly the navigation blue dot snaps to the calculated route during navigation.
//  A higher value increases the snapping strength, making the blue dot stay closer to the route line.
//  When null, the snapping behavior is not used.
//
...
final class NavigationRoadSnapStrengthViewController: BaseFeatureViewController, UITextFieldDelegate {

    private let textField = UITextField()

    private enum StrengthValidationError: LocalizedError {
        case invalidNumber
        var errorDescription: String? {
            switch self {
            case .invalidNumber:
                return "Please enter a valid number, or leave it empty for nil."
            }
        }
    }
    ...
    override func saveBarButtonTapped() {
        ...
        if let value = value {
            ...
            Config.shared.diConfig.navigationRoadSnapStrength = kotlinValue
            Config.shared.saveChange([featureName: kotlinValue])
        } else {
            Config.shared.diConfig.navigationRoadSnapStrength = nil
    ...
    private func validate() -> (error: Error?, value: Double?) {
        ...
        guard let value = Double(text) else {
            return (StrengthValidationError.invalidNumber, nil)
        }
        return (nil, value)
    }
    ...
    private func inputRowView() -> UIView {
        ...
        let label = UILabel()
        label.text = "navigationRoadSnapStrength :"
