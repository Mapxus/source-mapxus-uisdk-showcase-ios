...
//  Default: nil
//  Description: Detailed message displayed when no navigation route is available.
//  Shown on the navigation preview page if no route can be found.
//  Supports multiple languages via [StringsWithLanguage].
//  When null, a default no-route message content is used.
//
...
final class NoRouteAvailableMessageViewController: BaseFeatureViewController {
    ...
    private enum StringsValidationError: LocalizedError {
        ...
        var errorDescription: String? {
            switch self {
            case .atLeastOneRequired:
                return "Please fill in at least one language value."
            }
        }
    }
    ...
    override func saveBarButtonTapped() {
        ...
        if segmentControl.selectedSegmentIndex == 0 {
            Config.shared.diConfig.noRouteAvailableMessage = nil
            Config.shared.saveChange([featureName: nil as StringsWithLanguage?])
        } else if let strings = strings {
            Config.shared.diConfig.noRouteAvailableMessage = strings
    ...
    private func selectorRowView() -> UIView {
        ...
        let label = UILabel()
        label.text = "noRouteAvailableMessage :"
