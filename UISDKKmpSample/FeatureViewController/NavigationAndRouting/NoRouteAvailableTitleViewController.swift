...
//  Default: nil
//  Description: Title displayed when no navigation route is available.
//  Shown on the navigation preview page if no route can be found.
//  Supports multiple languages via [StringsWithLanguage].
//  When null, a default no-route available message is displayed.
//
...
final class NoRouteAvailableTitleViewController: BaseFeatureViewController {
    ...
    override func saveBarButtonTapped() {
        ...
        if segmentControl.selectedSegmentIndex == 0 {
            Config.shared.diConfig.noRouteAvailableTitle = nil
            Config.shared.saveChange([featureName: nil as StringsWithLanguage?])
        } else if let strings = strings {
            Config.shared.diConfig.noRouteAvailableTitle = strings
    ...
    private func selectorRowView() -> UIView {
        ...
        let label = UILabel()
        label.text = "noRouteAvailableTitle :"
