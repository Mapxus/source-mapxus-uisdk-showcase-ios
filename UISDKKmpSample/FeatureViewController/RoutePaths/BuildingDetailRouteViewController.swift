...
final class BuildingDetailRouteViewController: BaseFeatureViewController {

    private let buildingIdField: UITextField = {
        let t = UITextField()
        t.borderStyle = .roundedRect
        t.placeholder = "buildingId"
    ...
    override func saveBarButtonTapped() {
        guard let route = parseRoute() else {
            let alert = UIAlertController(title: "Invalid Input", message: "Please enter a buildingId.", preferredStyle: .alert)
    ...
    func parseRoute() -> BuildingDetailRoute? {
        let buildingId = (buildingIdField.text ?? "").trimmingCharacters(in: .whitespacesAndNewlines)
        guard !buildingId.isEmpty else { return nil }
        return BuildingDetailRoute(buildingId: buildingId)
    }
    ...
    private func buildFormView() -> UIView {
        ...
        titleLabel.text = "buildingId"
        titleLabel.font = .systemFont(ofSize: 15, weight: .semibold)
        titleLabel.textColor = .label

        stack.addArrangedSubview(titleLabel)
        stack.addArrangedSubview(buildingIdField)
        return stack
    }
}
